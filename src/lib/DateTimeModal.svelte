							...
							<div class="mt-6">
								<Flatpickr {options} bind:value={dateValue} name="date" element="#my-picker">
									<div class="flatpickr relative" id="my-picker">
										<input
											class="rounded-lg border border-gray-300 bg-gray-50 p-2 pl-4 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500"
											type="text"
											placeholder="Escolha uma data..."
											data-input />
										<button class="absolute top-2 right-2" title="toggle" data-toggle>
								...
								<select
									...
									class="mt-2 block w-full appearance-none rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500">
									<option selected disabled>Escolha uma hora...</option>
									{#each availableDates as date}
										<option value={date}>{`${date.getUTCHours()}:${date.getUTCMinutes()}.${date.getUTCSeconds()}`}</option>
									{/each}
								</select>
							</div>
						</div>
