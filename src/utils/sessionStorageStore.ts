// Adapted from https://github.com/joshnuss/svelte-local-storage-store/blob/master/index.ts
// To work for sessionStorage instead of localStorage
...
export function writable<T>(key: string, initialValue: T, options?: Options<T>): Writable<T> {
	const browser = typeof sessionStorage != 'undefined' && typeof window != 'undefined';
	...
	if (!stores[key]) {
		const store = internal(initialValue, (set) => {
			const json = browser ? sessionStorage.getItem(key) : null;
